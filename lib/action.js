...
export async function submitForm(prevState, formData) {
  ...
  // 서버 측의 유효성 검사
  if (
    isValidText(prescription.title) ||
    isValidText(prescription.summary) ||
    isValidText(prescription.instruction) ||
    isValidText(prescription.creator_email) ||
    isValidText(prescription.creator) ||
    !prescription.creator_email.includes('@') ||
    !prescription.image || prescription.image.size === 0
  ) {
    // response를 반환
   return {
